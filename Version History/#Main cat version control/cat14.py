...
def movement(current,moves):
    ...
    return nextgrid
def hungrymovement(current,moves):
    nextgrid = np.zeros((MAXROW,MAXCOL), dtype=int)
    for r in range(MAXROW):
        for c in range(MAXCOL):
            for i in range(current[r,c]):
...
def map_cat(CATNO):
    paths = ['cat2.png','cat2.png','cat2.png','cat2.png','cat2.png','cat2.png']
    xlist = []
    ylist = []
    slist = []
    for r in range(MAXROW):
...
#    unhappyarrays = np.zeros((MAXROW, MAXCOL), dtype=int)
#    happy = 100
    water, food, bed, boxes, boundary = sources()
...
#       kittenarrays = kittennext
        ...
        print(happy)
        if happy < 70:
            catnext = hungrymovement(catarrays,catmove)
            happy = 100


#       if age1 < 12:
#            map_kitten(kittenarrays)
#        else:
        map_cat(catarrays) 
